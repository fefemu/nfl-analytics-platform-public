...
- Ordinary weekly comparisons remain explicitly marked as latest pre-kickoff market
  movement, with `is_closing_snapshot = false` and `is_clv = false`.
- A dedicated `kickoff_capture_*` observation is eligible as a closing snapshot only
  when it was fetched 45–75 minutes before kickoff; this makes the CLV label auditable.
