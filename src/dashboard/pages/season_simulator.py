...
def render_season_simulator(
    ...
    metrics[0].metric(tr(language, "teams"), len(standings))
    metrics[1].metric(
    ...
    metrics[2].metric(
        ...
        str(leader["team"]),
        f"{_number(leader['expected_wins'], language)} "
        + ("várható győzelem" if language == "HU" else "expected wins"),
        delta_color="off",
    )
    ...
    with left:
        st.markdown("### " + ("League outlook" if language == "EN" else "Teljes liga előrejelzése"))
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "expected_wins": st.column_config.NumberColumn(
                    ...
                    help=("A szimulációk során elért győzelmek átlagos száma." if language == "HU" else "Average wins across all simulated seasons."),
                ),
                "median_wins": st.column_config.NumberColumn(
                    "Medián" if language == "HU" else "Median", format="%.1f",
                    help=("A szimulációk fele ennél kevesebb, fele ennél több győzelemmel zárult." if language == "HU" else "Half of simulations finished below and half above this value."),
                ),
                "p10_p90_range": st.column_config.TextColumn(
                    "P10–P90",
                    help=("A szimulált eredmények középső 80%-ának tartománya." if language == "HU" else "Range containing the middle 80% of simulated outcomes."),
                ),
                "most_likely_wins": st.column_config.NumberColumn(
                    "Módusz" if language == "HU" else "Mode", format="%d",
                    help=("A szimulációkban leggyakrabban előforduló győzelemszám." if language == "HU" else "Most frequently occurring win total in the simulations."),
                ),
            },
        )
    with right:
        ...
        if not benchmark.empty and team in set(benchmark["team"]):
            ...
            compare[0].metric("Dinamikus Elo" if language == "HU" else "Dynamic Elo", _number(comparison["dynamic_expected_wins"], language, 2), help=dynamic_help)
            compare[1].metric("Rögzített Elo" if language == "HU" else "Frozen Elo", _number(comparison["frozen_expected_wins"], language, 2), help=frozen_help)
            compare[2].metric(
                "Eltérés" if language == "HU" else "Difference",
                ("+" if comparison["expected_wins_delta"] >= 0 else "") + _number(comparison["expected_wins_delta"], language, 2),
                delta_color="off", help=difference_help,
            )
