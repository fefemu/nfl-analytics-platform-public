...
Totals Fallback Expanding-Window Backtest

Purpose:
    Compare production-available totals fallback feature
    sets and Ridge alpha values across multiple
    chronological validation seasons.
...
def evaluate_totals_fallback_expanding_window(
...
def run_totals_fallback_expanding_window_backtest(
    ...
    logger.info(
        "Totals fallback expanding-window backtest "
        "completed with %s folds and %s candidate "
        "settings.",
...
def main() -> None:
    """Run and print the totals fallback backtest."""

    summary_results, fold_results = (
        run_totals_fallback_expanding_window_backtest()
    )
