...
Final Spread Model Holdout Evaluation

Purpose:
    Evaluate the locked Elo + QB Ridge spread model
    exactly once on the untouched 2025 holdout.
...
def load_spread_holdout_data(
    ...
    """Load development and holdout spread data."""

    feature_select = ",\n            ".join(
        f"dataset.{feature_name}"
        for feature_name in ELO_QB_SPREAD_FEATURES
    )
...
def prepare_spread_holdout_sample(
    data: pd.DataFrame,
    ...
    if unexpected_splits:
        raise ValueError(
            "Unexpected spread split names: "
            + ", ".join(unexpected_splits)
        )

    complete_mask = (
        data[SPREAD_TARGET_COLUMN].notna()
        & data[
    ...
    if development_data.empty:
        raise RuntimeError(
            "No complete spread development games "
            "are available."
        )

    if holdout_data.empty:
        raise RuntimeError(
            "No complete spread holdout games "
            "are available."
        )
    ...
    if (
        ...
        raise ValueError(
            "Spread development seasons must precede "
            "the holdout season."
        )
...
def evaluate_locked_spread_holdout(
    ...
    if ridge_alpha != FINAL_RIDGE_ALPHA:
        raise ValueError(
            "The final spread Ridge alpha is locked "
...
def run_locked_spread_holdout_evaluation(
    database_file: Path = DATABASE_FILE,
) -> pd.DataFrame:
    """Run the final spread holdout evaluation."""
    ...
    logger.info(
        "Locked spread model evaluated once on "
...
def main() -> None:
    ...
    results = (
        run_locked_spread_holdout_evaluation()
    )

    print("\nFINAL SPREAD HOLDOUT RESULTS\n")
