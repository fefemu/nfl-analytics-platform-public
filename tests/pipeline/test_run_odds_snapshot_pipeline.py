"""Tests for the offline odds snapshot pipeline."""

from pathlib import Path

import pytest

from src.pipeline.run_odds_snapshot_pipeline import (
    run_odds_snapshot_pipeline,
)


def test_snapshot_pipeline_calls_steps_in_order(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """Rebuild all odds layers in dependency order."""
    ...
    monkeypatch.setattr("src.pipeline.run_odds_snapshot_pipeline.build_current_moneyline_value", fake_moneyline)
    monkeypatch.setattr("src.pipeline.run_odds_snapshot_pipeline.build_current_spread_value", fake_spread)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("best", ["raw", "processed", "best"]),
        (
            "bridge",
            ["raw", "processed", "best", "bridge"],
        ),
        (
            "market_board",
            [
        ...
        ("moneyline", ["raw", "processed", "best", "bridge", "market_board", "moneyline"]),
        ("spread", ["raw", "processed", "best", "bridge", "market_board", "moneyline", "spread"]),
        ("totals", ["raw", "processed", "best", "bridge", "market_board", "moneyline", "spread", "totals"]),
        ("board", ["raw", "processed", "best", "bridge", "market_board", "moneyline", "spread", "totals", "board"]),
    ],
)
def test_snapshot_pipeline_stops_and_propagates_failure(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    failure_step: str,
    expected_calls: list[str],
) -> None:
    """Stop at the failing offline pipeline step."""
