...
def board(snapshot="s1", fetched="2026-09-01T12:00:00Z", odds=2.0, positive=True):
    ...
    row.update({
        ...
        "model_version": "1", "prediction_mode": "mode", "model_probability": 0.6,
...
def test_prepare_rejects_only_started_games():
    ...
    with pytest.raises(RuntimeError, match="No future"):
        prepare_forward_archive_rows(started, "run")


def test_persistence_is_idempotent_and_clv_uses_later_snapshot():
    first = prepare_forward_archive_rows(board(), "run1")
    second = prepare_forward_archive_rows(board("s2", "2026-09-05T12:00:00Z", 1.8, False), "run2")
